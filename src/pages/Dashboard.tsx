  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      ...
      if (profile) {
        ...
        // Fetch recommended quizzes based on interests
        if (profile.interests && profile.interests.length > 0) {
          const { data: recommended } = await supabase
            ...
            .order('created_at', { ascending: false })
            .limit(3);
          if (recommended) setRecommendedQuizzes(recommended);
        }
      }
    };
  ...
  return (
    <PageTransition className="min-h-screen p-6 md:p-12 max-w-7xl mx-auto">
      ...
      {recommendedQuizzes.length > 0 && (
        <div className="mb-12">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <h2 className={cn("text-2xl font-bold", isDark ? "text-white" : "text-gray-900")}>Recommended For You</h2>
